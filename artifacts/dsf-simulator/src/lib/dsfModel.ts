...
export const variableLegend = {
  ...
  theology: [
    ...
    { sym: "U", name: "Usury pressure", role: "Weighted extractive pressure (generalized; aquinas_unified at φ=0.5, ψ≡1 gives the old 0.5ρ+λ).", eq: "U = w_\\rho(1-\\varphi)\\psi_\\rho\\rho + w_\\lambda(\\psi_\\lambda\\lambda + D)" },
    { sym: "T", name: "Theological integrity", role: "Moral acceptability score, gated at U_max.", eq: "T = \\begin{cases}0 & U > U_{max}\\\\ (1-U) + \\mu\\eta(1 - U/U_{max}) & U \\le U_{max}\\end{cases}" },
    ...
    { sym: "\\varphi", name: "Claim contingency (φ_state)", role: "0 = guaranteed claim (mutuum pole), 1 = fully contingent (societas pole). Operative in U; φ_dyn is display-only.", eq: "" },
    { sym: "\\psi_i", name: "Substantiation of component i", role: "Share of the claim actually substantiated; the rest flows into D.", eq: "" },
    { sym: "D", name: "Unsubstantiated claim", role: "Charged as usury in every mode.", eq: "D = \\sum_i (1-\\psi_i)\\,\\text{comp}_i" },
    { sym: "U_{max}", name: "Licitness gate", role: "T = 0 above this usury pressure (founder decision, 2 Jul 2026).", eq: "" },
    { sym: "X", name: "Excess claim", role: "Claim above substantiated compensation — what the verdict predicates test.", eq: "X = (r-1) - (\\psi_\\delta\\delta + \\psi_\\pi\\pi)" },
    { sym: "\\Lambda", name: "Counterfactual growth", role: "Olivi committed-capital threshold: Λ_time (operative) vs Λ_ens (comparison).", eq: "\\Lambda_{time} = e^{g_{alt}T}-1,\\; \\Lambda_{ens} = (1+\\bar{y})^T-1" },
    { sym: "g", name: "Time-average growth rate", role: "What one path actually compounds at (ergodicity module).", eq: "g = \\mathbb{E}[\\ln(\\eta \\hat{M})]" },
    { sym: "\\hat{M}", name: "Realized multiple", role: "Random per-path multiple over the binomial survivor count.", eq: "\\hat{M} = \\dfrac{rk\\hat{S}}{N+(k-1)\\hat{S}}" },
    { sym: "\\hat{S}", name: "Realized survivors", role: "Binomial survivor count.", eq: "\\hat{S} \\sim \\mathrm{Bin}(N, p)" },
    { sym: "f^\\star", name: "Kelly-optimal stake fraction", role: "Deployment fraction that maximizes g(f).", eq: "" },
    { sym: "\\Delta g", name: "Growth drag", role: "Borrower's growth loss vs the free company (feeds φ_dyn).", eq: "\\varphi_{dyn} = 1 - \\Delta g_B/\\Delta g_A" },
